...
def _get_or_create_exam(user, course: Course, external_id: str, title: str) -> Exam:
    existing = Exam.objects.filter(user=user, external_id=external_id).first()
    if existing:
        return existing
...
def _resolve_models(user, model_names: List[str]) -> List:
    ...
    for name in model_names:
        ...
        if model is None:
            raise ValueError(f"Model '{name}' is not enabled for this user")
        resolved.append(model)
    return resolved


def _aggregate_status(evaluations) -> Tuple[str, int, int, int, int]:
...
def create_api_batch_evaluation(user, payload: Dict[str, Any]) -> Tuple[Any, int]:
